...
# Try to import pygetwindow, fall back gracefully if not available. The package
# raises `NotImplementedError` rather than `ImportError` on platforms where it
# has no bindings (currently Linux), so the except clause has to catch both
# — the agent is fully usable on Linux without window controls.
try:
