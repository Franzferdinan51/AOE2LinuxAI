"""Detection phase — owns the periodic screenshot/detect cycle.

Extracted from the executor so the perceive/act clocks can stay small. The
phase runs the SAHI-gated detection, classifies ownership of military units,
and keeps the global ``detected_entities`` slot fresh.
"""
...
from .io.input import get_game_window_rect
...
# Confidence at which the tracker is trusted to predict next-frame entities
# without running a fresh detection. Below this we always re-detect.
TRACKER_CONFIDENCE_THRESHOLD = 0.6
# Frame-diff threshold; a rescan whose hash matches the previous frame
# is skipped outright (a hard "nothing changed").
FRAME_DIFFER_THRESHOLD = 0.05
# Mid-turn rescan uses a lower JPEG quality than full detection to keep
# latency under budget when the cam-pan tracker is doing the heavy lifting.
RESCAN_SCREENSHOT_QUALITY = 50
# If a new detection returns fewer entities than this fraction of the last,
# the tracker resets (camera moved).
ENTITY_DROP_RATIO = 0.5
# Owned military entities to display in the per-turn context.
ENTITY_DISPLAY_LIMIT = 25

# Global slot the executor reads. ``set_detected_entities`` lives in
# ``game_loop`` so the import order doesn't matter.
...
_DETECTION_AVAILABLE: bool = True
try:
...
def _get_detector() -> Detector | None:
    """Build the configured detector; ``None`` means detection is disabled."""
