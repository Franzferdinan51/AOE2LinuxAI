...
from .executor import set_input_injector
...
async def main_async(args: _AgentArgs) -> None:
    ...
    # LM Studio's local server ignores auth, so the credential check is waived
    # there. The dummy key the wire substitutes is never sent to the server.
    if wire_name != "lmstudio" and not config.llm_api_key:
        log.error("missing_api_key", message=f"Set {KEY_ENV}")
        sys.exit(1)
    if wire_name == "lmstudio" and not config.model:
        log.error(
            "missing_model",
            message=(
                "Set AOE2_MODEL to the LM Studio model id "
                "(use `lms ls` or the LM Studio developer tab)."
            ),
        )
        sys.exit(2)
    ...
    log.info("provider_created", wire=wire_name, model=config.model, endpoint=config.llm_base_url or "<default>")

    # Pick an input backend once, lazily, before the first action runs.
    set_input_injector()
