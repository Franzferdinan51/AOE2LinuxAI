...
class VillagerTargets(BaseModel):
    """Villagers wanted per resource. Fixed keys, not a dict: OpenAI structured outputs 400s on an open object."""
...
def get_default_goals(turn: int = 0) -> list[Goal]:
    ...
    return [
        Goal(name="Queue villagers", type="local", metric="population", target=10, priority=9, created_turn=turn),
        Goal(name="Gather food", type="local", metric="food", target=200, priority=8, created_turn=turn),
        Goal(name="Advance to Feudal Age", type="global", metric="age", target="Feudal Age", priority=4, created_turn=turn),
    ]
...
__all__ = [
    "StrategistProvider", "StrategistResponse", "VillagerTargets",
    "as_allocation", "get_default_goals", "read_hud_readings",
]
