...
An AI agent that plays Age of Empires 2: Definitive Edition using a two-tier LLM architecture: a strategist reads the resource bar via local OCR and sets goals, an executor reads YOLO entity detections and executes actions. Both LLM tiers are text-only, so no image is ever sent to the model. The vendor is a config value — `gpt-5.6-luna` on OpenAI by default, with Claude, OpenCode Zen, or a local LM Studio server each one env var away.

## Why this fork

The upstream project ([dobosmarton/aoe2-agent](https://github.com/dobosmarton/aoe2-agent)) is Windows-first: the input layer is built on `pyautogui` (Win32) and `pygetwindow` (Win32 window enumeration). This fork keeps the upstream architecture intact and replaces only the platform-specific seams:

- **Linux-native input.** `xdotool` for X11 sessions, `ydotool` for Wayland sessions (Hyprland, Sway, KDE, GNOME). Both run through Steam + Proton, where AoE2 DE itself plays. `pyautogui` survives as an opt-in fallback under the `[windows]` extra so the upstream code path still works.
- **LM Studio wire.** LM Studio's local OpenAI-compatible server (`http://localhost:1234/v1`) is added as a fourth adapter alongside OpenAI, OpenCode Zen, and Anthropic. No API key required.
- **Cross-platform tests.** Three new test files exercise the LM Studio wire, the Linux input backends, and the full perceive → LLM → input pipeline end-to-end with a threadpool mock server.

The vendor-neutral config-var approach (`AOE2_LLM_WIRE=<adapter>`) is preserved; the existing adapter set is now a four-way fan-out rather than three.
...
9. **Act** — Execute mouse clicks / keyboard presses via the selected input backend (see [Input backends](#input-backends))
10. **Remember** — Update memory, evaluate goal progress, compute rewards

## Requirements

- **Linux** (X11 or Wayland) with AoE2:DE running under Steam + Proton (no native Linux build exists; AoE2 DE is rated **Gold** on Valve's Proton compatibility tier)
- **Python 3.11+** (x64, not ARM64)
- One of:
  - An OpenAI / Anthropic / OpenCode Zen API key (`AOE2_LLM_API_KEY`), or
  - A running LM Studio server with a loaded model (no key needed)

### Linux input tools

The agent composes `xdotool` / `ydotool` calls. Both are small shell-out binaries; install via your package manager:
...
sudo dnf install xdotool ydotool     # Fedora: ydotool is in Fedora 38+
```

Hyprland and Sway both go through `ydotoold` (the daemon side of `ydotool`). Start it once per user session:

```bash
# systemd user unit (recommended; survives reboots)
systemctl --user enable --now ydotoold.socket ydotoold.service
...
X11 only needs `xdotool`; the agent talks to whichever `$DISPLAY` is set.

Steam / Proton runs the game itself (Proton ≥ 9.x is Gold-rated for AoE2 DE) and the input tools compose the same shell-out over the Wine/Proton boundary — no extra wiring.

## Installation

The repo is a polyglot monorepo:

- **Python (`uv workspace`)** — 10 workspace members. Deployable units live in `apps/` (agent, api, arena, autoresearch, detection-server, training-api); reusable libraries live in `packages/` (core, data, detection, evaluation). One `uv sync` installs every member editable into a shared `.venv/`, plus the dev dependency-group (ruff, basedpyright, pytest, fakeredis, hypothesis, pre-commit).
- **TypeScript (`bun workspace`)** — two frontend apps (`apps/dashboard` Vite/React, `apps/landing` Astro) share a single root `bun.lock` and hoisted `node_modules/`. One `bun install` from the repo root resolves both apps.
...
# Run a specific package's entry point directly:
...
### Windows install (legacy)

Windows remains supported. Install the `[windows]` extra on top of the standard sync to pull in `pyautogui` and `pygetwindow`, then set `AOE2_INPUT_BACKEND=pyautogui`:

```bash
uv sync --package gameplay-agent --extra windows
export AOE2_INPUT_BACKEND=pyautogui
just agent
```

## Configuration

The project reads configuration from environment variables. For local development we keep them in a gitignored `.env` file (loaded by `docker compose` automatically, and by the agent when launched via `just agent`). A documented template lives at `.env.example`.
...
- `AOE2_LLM_API_KEY` — for the OpenAI / Anthropic / OpenCode Zen wires, or
- `AOE2_MODEL` + a running LM Studio server — for the LM Studio wire (no key needed).

Every other variable in `.env.example` is for the **Synthetic Arena infrastructure** (Langfuse + MinIO + ClickHouse + Redis + Postgres) and is only consumed by `just arena-infra-up`. If you're not running the arena stack yet, leaving those blank is fine.

### Vendor-neutral wiring

The agent defaults to GPT-5.6 Luna on the OpenAI API. Each adapter supplies its own endpoint, so switching vendor is one variable: `AOE2_LLM_WIRE=lmstudio` for a local LM Studio server, `AOE2_LLM_WIRE=zen` for OpenCode Zen, or `AOE2_LLM_WIRE=anthropic` plus an `AOE2_MODEL` Claude model for Claude. The key is `AOE2_LLM_API_KEY` for the three remote wires; LM Studio does not check the header (see [LM Studio](#lm-studio) below).

The name tolerates case and surrounding whitespace, so `LMSTUDIO` and `" lmstudio "` both work. An unrecognised name stops the process at startup rather than falling back to a default, because a silent fallback means playing a whole game on a vendor you did not choose:

```
ValueError: unknown AOE2_LLM_WIRE='zzz'; expected one of 'anthropic', 'lmstudio', 'openai', 'zen'
```

| Env Var | Default | Purpose |
|---------|---------|---------|
| `AOE2_LLM_API_KEY` | — | Model API authentication (required for remote wires; ignored by `lmstudio`) |
| `AOE2_LLM_WIRE` | `openai` | Adapter: `openai`, `zen` (OpenCode Zen), `anthropic`, or `lmstudio` |
| `AOE2_LLM_BASE_URL` | — | Endpoint override; empty uses the adapter's own |
| `AOE2_MODEL` | `gpt-5.6-luna` | Executor model (fast; runs every turn). For LM Studio, set to a loaded model id |
...
| `AOE2_INPUT_BACKEND` | `auto` | See [Input backends](#input-backends) |

## Input backends

The input layer is a `Protocol` (`gameplay_agent/io/input.py:InputInjector`) with two methods each for click, motion, key, drag, scroll. Three real backends implement it; a fourth is a recording stub for tests:

| Backend | Value | Requires | Notes |
|---|---|---|---|
| Auto | `auto` (default) | none beyond `xdotool` or `ydotool` installed | Reads `XDG_SESSION_TYPE` and `$PATH`; prefers Wayland, falls back to X11, then to `pyautogui` |
| X11 | `x11` | `xdotool` on `$PATH`, `$DISPLAY` set | `X11InputInjector`; `xdotool mousemove --sync` blocks until the server parks the cursor, so back-to-back screenshots don't race |
| Wayland | `wayland` | `ydotool` on `$PATH`, `ydotoold` running | `WaylandInputInjector`; covers Hyprland, Sway, KDE Wayland, GNOME Wayland |
| Silent | `silent` | nothing | `SilentInputInjector`; records every call into a log, never injects. Used by tests and `tests/test_loop_lmstudio.py` |
| pyautogui | `pyautogui` | `pyautogui` + `pygetwindow` (Windows extras) | Original upstream backend; opt-in via `uv sync --package gameplay-agent --extra windows` |

Override with `AOE2_INPUT_BACKEND=<value>` (or pass through the `--input-backend` flag if your script wraps the agent). The auto picker ignores missing tools silently — if neither `xdotool` nor `ydotool` is present and `XDG_SESSION_TYPE` is unset, the loop falls back to `pyautogui`, which then errors out with whatever pyautogui itself raises. Install at least one binary to avoid that.

**Mouse motion** is in absolute screen pixels. The X11 backend serialises each click as `mousemove --sync` + `click 1` (left) / `click 3` (right); the Wayland backend uses `ydotool mousemove --absolute -- X Y` and a single-byte `click` mask (`0xC0` left, `0xC2` right). Modifiers are cleared on every key press (`xdotool --clearmodifiers`) to avoid sticky Ctrl/Shift from misdirected hotkeys.

**Drag emulation.** `xdotool` lacks a `mousedown→mouseup` over duration primitive, so the X11 backend slides the cursor between waypoints manually before issuing the press pair. `ydotool` has the right primitives; the Wayland backend uses them directly.

## LM Studio

LM Studio runs an OpenAI-compatible Chat Completions server on `http://localhost:1234/v1` by default. The `lmstudio` wire inherits every transport detail from `OpenAIWire` (`providers/wire_lmstudio.py:LMStudioWire`) and only swaps the defaults: no auth header, localhost endpoint, a dummy API key string the SDK requires to be non-empty. LM Studio ignores the `Authorization` header on the wire, so any opaque string works — `lm-studio` is the documented sentinel.
...
The recipe sets `AOE2_LLM_WIRE=lmstudio`, points `AOE2_MODEL` at the loaded model id, and forwards any extra args (`--iterations 20`, `--test`, etc.) to the agent.

### Manual setup

Start the LM Studio developer server:

```bash
# In the LM Studio desktop app: open the Developer tab and click "Start Server".
# Or from the CLI:
lms server start
```

Then set three env vars:

```bash
export AOE2_LLM_WIRE=lmstudio
export AOE2_MODEL=qwen2.5-7b-instruct          # the model id LM Studio shows in its UI
export AOE2_LLM_BASE_URL=http://localhost:1234/v1   # only needed if the server is on a different host/port
just agent
```

`AOE2_LLM_API_KEY` is **not** required for `lmstudio` and is not sent to the server. If the model field is empty the agent refuses to start with `missing_model` — set `AOE2_MODEL` to the loaded model id. List what's loaded from Python without spinning up the agent:

```python
from gameplay_agent.providers.wire_lmstudio import list_loaded_models

models = list_loaded_models()                       # -> list of {"id": ..., "object": "model", ...}
print([m["id"] for m in models])
```

`list_loaded_models()` hits `GET {base_url}/models` (OpenAI's models endpoint) and returns the parsed `data` array. `requests` is imported lazily inside the helper, so it is cheap to call from tooling without pulling the OpenAI SDK on the user's behalf.

If the server lives on another host (a remote GPU box, a tunnel), set `AOE2_LLM_BASE_URL=http://gpu-box.local:1234/v1` — the wire sends everything else to the URL as-is.

### Event broker backend (Phase C)

The arena runs every event through an `EventBroker` (see `docs/design/event-broker-architecture.md`). The default in-process broker has zero external dependencies and is fine for single-machine work — local development, CI, and `just arena-smoke` all use it implicitly.
...
Install the Redis client when using this backend (`fakeredis` covers CI tests; real-Redis local work needs the `broker-redis` extra):
...
- **Pin model snapshots.** Set `AOE2_MODEL` and `AOE2_STRATEGIST_MODEL` to a dated snapshot rather than the floating family alias. Floating tags can move under you between runs. `providers/pricing.py` keys on the exact name, so add the dated name there too or every cost figure reads $0.00. For LM Studio, "pinned" means whichever model id the server is currently serving — restart the server with a fixed model loaded if you need byte-stable prompts.
- **The defaults follow the wire.** A model name belongs to its vendor, so `AOE2_LLM_WIRE=anthropic` defaults the executor to `claude-haiku-4-5` and the strategist to `claude-sonnet-5`. `config._MODELS_BY_WIRE` holds the table; an env override still wins per role. The `lmstudio` row falls back to an empty default, on purpose — there is no model id the LM Studio server can be assumed to have.

### Synthetic Arena infrastructure (optional)

Required only when bringing up the Docker stack (`just arena-infra-up`). All seven variables below must be set to non-empty values — Langfuse refuses to boot with empty secrets. **Never commit the populated `.env`** (it's gitignored).
...
**One-shot generator** — paste this once after copying `.env.example` to `.env`, then fill the output back into `.env` (or pipe directly with a script of your choosing):
...
# Run the agent against a local LM Studio server
just agent-lmstudio

# Run the agent against LM Studio, but only 20 iterations
just agent-lmstudio --iterations 20

# Force a specific input backend
AOE2_INPUT_BACKEND=x11 just agent-lmstudio
AOE2_INPUT_BACKEND=wayland just agent-lmstudio
...
Each Python workspace member has its own `pyproject.toml` declaring deps + optional extras. The dependency graph is one-way (`apps/` → `packages/`, never the reverse), enforced by uv at install time via `[tool.uv.sources]` in the workspace root. uv resolves members **by package name**, not by directory path — so internal dependencies like `dependencies = ["core", "detection"]` work regardless of which subdirectory the member lives in.
...
### Input abstraction (`gameplay_agent/io/`)

`input.py` defines `InputInjector` as a `@runtime_checkable` `Protocol` with seven methods (`click`, `right_click`, `move_to`, `press`, `hotkey`, `drag`, `scroll`). The executor and tests hold an `InputInjector` reference and never import a backend directly — `set_input_injector()` in `executor.py` calls `select_input_backend()` once at startup, which honours `AOE2_INPUT_BACKEND` and falls back to `auto` resolution. `linux_input.py` carries `X11InputInjector` / `WaylandInputInjector` / `SilentInputInjector`; `pyautogui_input.py` carries the upstream `PyautoguiInjector` for Windows and macOS. `io/__init__.py` re-exports the protocol plus every backend so callers depend only on the package.

### LM Studio wire (`gameplay_agent/providers/wire_lmstudio.py`)

`LMStudioWire(OpenAIWire)` is a four-line override of the OpenAI wire: the default `base_url` becomes `http://localhost:1234/v1`, the default `api_key` becomes the documented `lm-studio` sentinel. Every transport detail — request shape, retry, structured-output rendering, tool-loop — comes from `OpenAIWire`, so behaviour stays aligned if either endpoint diverges upstream. `list_loaded_models()` hits `GET {base_url}/models` and returns the parsed `data` list.
...
Three new test files cover the Linux + LM Studio surface:

| Test | Verifies |
|---|---|
| `tests/test_lmstudio_wire.py` | `LMStudioWire` is registered by `make_wire` under `lmstudio`; the default `base_url` is `http://localhost:1234/v1`; a real OpenAI-shaped POST reaches the LM Studio mock server and renders an OpenAI-shaped body, not an Anthropic one. The mock is a threadpool `http.server` recording every request and returning one canned ChatCompletion. |
| `tests/test_input_backend_linux.py` | The Linux-native input backends invoke their respective driver binaries (`xdotool`, `ydotool`) with the correct arguments and never touch the real device. Every external command is shadowed by a stub script in `$PATH` that records its argv into a temp log; `Xvfb` is used opportunistically. |
| `tests/test_loop_lmstudio.py` | End-to-end perceive → LLM → input pipeline. Stands up an in-process LM Studio mock (`/v1/chat/completions` + `/v1/models`), replaces the agent's capture + detection seams with a synthetic `WorldState` renderer, and exercises the production code path. Goes through `act()` (the tool-loop path) rather than `parse_structured()` because LM Studio's local server does not implement the OpenAI structured-output schema endpoint. |

Run them:

```bash
just test tests/test_lmstudio_wire.py
just test tests/test_input_backend_linux.py
just test tests/test_loop_lmstudio.py
```
...
- **Operational checklists**: [Runbooks](docs/runbooks/) (Redis ops, switching broker backend, debug a stuck fork, Linux install).
- **Deployment**: [deployment-guide.md](docs/deployment-guide.md) (Linux + Steam/Proton first-time setup, macOS detection host, Windows VM bring-up).
