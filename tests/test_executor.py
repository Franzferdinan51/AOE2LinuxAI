...
class _FakeInput:
    ...
    Records each call in the legacy pyautogui shape — `(name, positional_args,
    kwargs)` — so historic test assertions (`("hotkey", ("ctrl", "1"), {})`)
    keep matching after the executor moved from `pyautogui` to the
    `InputInjector` seam. The actual seam this fixture patches is
    `executor._input`.
    """
    ...
    # Protocol surface — every method the executor calls routes through here.
    ...
    def hotkey(self, modifiers: list[str], key: str) -> None:
        # pyautogui's signature is hotkey(*keys); flatten for parity.
        self.calls.append(("hotkey", (*modifiers, key), {}))
    ...
    def drag(
        ...
        # pyautogui recorded drag(dx, dy, duration=...); flatten to dx, dy.
        dx = end_xy[0] - start_xy[0]
        dy = end_xy[1] - start_xy[1]
        self.calls.append(("drag", (dx, dy), {"duration": duration}))
...
# Alias kept so historical callsites / test classes that import the legacy
# name continue to work. The executor no longer touches `pyautogui` at all —
# only the InputInjector seam is exercised.
_FakePyautogui = _FakeInput
...
@pytest.fixture
def fake_pyautogui(monkeypatch: pytest.MonkeyPatch, clock: _Clock) -> _FakeInput:
    """Replace `executor._input` with a recorder; stub side-effecty deps.

    Named `fake_pyautogui` for legacy fixture compat — the test assertions use
    the same `calls` list and `names()` surface, but the seam they patch is
    the executor's `InputInjector`, not pyautogui itself.
    """
    fake = _FakeInput()
    monkeypatch.setattr(ex, "_input", fake)
