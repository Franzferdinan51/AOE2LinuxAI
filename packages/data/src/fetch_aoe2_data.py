...
UNIT_TYPE_MAP = {
    0: "archer", 1: "artifact", 2: "trade_boat", 3: "building", 4: "civilian",
    6: "infantry", 8: "cavalry", 11: "building", 12: "cavalry", 13: "siege",
    14: "predator", 18: "monk", 19: "trade_unit", 22: "infantry", 23: "livestock",
    30: "tower", 36: "ship", 44: "ship", 51: "wall", 52: "resource",
    53: "resource", 54: "resource", 56: "resource", 57: "boar", 58: "sheep",
}
...
def fetch_halfon(url: str) -> HalfonResponse:
    print(f"  Fetching {url}...")
    req = urllib.request.Request(url, headers={"User-Agent": "aoe2-llm-arena/1.0"})
    with urllib.request.urlopen(req, timeout=30) as response:
        raw = cast("bytes", response.read())
    return HalfonResponse.model_validate_json(raw)


def _entity_to_record(entity_id: int, entity: HalfonEntity) -> dict[str, object]:
    return {
        "id": entity_id,
        ...
        "melee_armor": entity.melee_armor,
        "pierce_armor": entity.pierce_armor,
        "line_of_sight": entity.line_of_sight,
        "garrison_capacity": entity.garrison_capacity,
        "cost": {
            "food": entity.cost.food,
            "wood": entity.cost.wood,
            "gold": entity.cost.gold,
            "stone": entity.cost.stone,
        },
    }


def process_halfon_data(payload: HalfonResponse) -> tuple[list[dict], list[dict]]:
    units: list[dict] = []
    buildings: list[dict] = []
    for id_str, entity in payload.units_buildings.items():
        try:
            ...
            continue
        if (
            not entity.display_name
            or entity.display_name.startswith("OLD-")
            or entity.hit_points <= 0
        ):
            continue
        record = _entity_to_record(entity_id, entity)
        is_building = (
            entity.type == 80
            or entity.class_ in (3, 11, 20, 21, 30, 51)
            or _is_building_by_name(entity.display_name)
        )
        if is_building:
            record["category"] = _categorize_building(entity.display_name)
            buildings.append(record)
        else:
            record["class"] = UNIT_TYPE_MAP.get(entity.class_, "unknown")
            record["total_cost"] = (
...
def _is_building_by_name(name: str) -> bool:
    ...
    keywords = [
        "tower", "castle", "wall", "gate", "house", "mill", "camp", "dock",
        "range", "stable", "barracks", "center", "monastery", "market",
        "university", "blacksmith", "wonder", "outpost", "workshop", "trap",
        "farm", "krepost", "donjon", "harbor", "folwark", "mule cart",
        "caravanserai", "feitoria",
    ]
...
def _categorize_building(name: str) -> str:
    ...
    economic = [
        "house", "mill", "camp", "dock", "market", "farm", "center",
        "blacksmith", "monastery", "university", "feitoria", "folwark",
    ]
    if any(k in name_lower for k in military):
        return "military"
    if any(k in name_lower for k in defensive):
        return "defensive"
    if any(k in name_lower for k in economic):
        return "economic"
    return "other"
...
def _parse_civ_bonuses(help_text: str) -> list[str]:
    ...
    for line in lines:
        ...
        if not line:
            continue
        if line.startswith(("\u2022", "\u2022")):
            bonus = line.removeprefix("\u2022").removeprefix("\u2022").strip()
            if bonus:
...
def main() -> int:
    ...
    try:
        halfon_payload = fetch_halfon(HALFON_URL)
        units, buildings = process_halfon_data(halfon_payload)
